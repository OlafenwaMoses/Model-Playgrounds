...
class actionThreadSqueezeNet(threading.Thread):
    ...
    def run(self):
        ...
        import SqueezenetPrediction
        result = SqueezenetPrediction.predict(self.path, self.model_path, self.index_file_path, self.main_ui) # This calls the SqueezeNet Prediction Class
...
class Mainapp(wx.Frame):
    ...
    prediction_ongoing = False  # To check and avoid attempt to initiate precdictions before another is done
    ...
    def Action(self):
        ...
        # Image Banner
        banner_bitmap1 = wx.Bitmap("PROGRAM_INSTALL_FULLPATH\\SqueezeNet.png")
        ...
        # The button to call the About SqueezeNet dialog
        about_squeezenet = wx.Button(left_menu, -1, "About SqueezeNet")
        about_squeezenet.SetPosition((20, 285))
        about_squeezenet.SetSize((200, 30))
        about_squeezenet_bitmap = wx.Bitmap("PROGRAM_INSTALL_FULLPATH\\info.png")
        about_squeezenet.SetBitmap(about_squeezenet_bitmap, wx.LEFT)
        self.Bind(wx.EVT_BUTTON, self.aboutSqueezenet, about_squeezenet)

        # The button to call the About dialog
        about_button = wx.Button(left_menu, -1, "About SqueezeNet Playground")
    ...
    def processPicture(self, path, model_path, index_file_path):
        ...
        self.magic_collection[1].SetValue(
            "Picture is currently being processed. May take a few minutes. Wait for the result! \nLoading.....")
        # Thread to initiate image prediction is called
        action = actionThreadSqueezeNet(path, model_path, index_file_path, self)
        action.start()
